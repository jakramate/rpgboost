function [fv, dfv] = grad_theta_log(w, F, h, x, y, lambda, k, reg_type)
...
fv = sum(log(1 + exp(ht)))/n + reg_fv;


% computing the gradients for 'w'
dfv    = zeros(length(w),1);
% precomputing the common term
cterm  = 1 + exp(-ht);   % dividing the original derivative with exp(ht) yields this common term
